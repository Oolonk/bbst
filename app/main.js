...
if(process.platform !== "win32"){
	if(!fs.existsSync(path.join(electron.APP.getPath("home"), 'Beyblade Stream Tool'))){
...
electron.on("ready", async () => { // programm is ready
	APPRES = global.APPRES = (await getClientSetting("resPath")) || path.join(electron.APP.getPath("home"), 'Beyblade Stream Tool');

	// make sure everything is alright
	await ensure(APPRES, APPROOT, APPUSERDATA);

	database.setPath(APPRES);
	database.newDb(['dbstruct', 'player', 'country', 'team', 'match', 'pride', 'region']);
	await database.load();
