...
Server.prototype.start = async function start() {
	...
	try{
		...
		this.bonjour.publish({ name: `${os.hostname()}-${(Math.random() + 1).toString(36).substring(7)}`, type: 'bboverlay', port: this.port, txt: this.about });
	} catch (err){
	console.error(err)
	}
	...
	this.server.get('/:filename', (req, res, next) => {
		try {
			...
			res.write('<title>BBOverlay | ' + (this.themeManifest.name || this.theme) + ' | ' + req.params.filename + ' ' + (this.themeManifest.resolution ? '[' + self.themeManifest.resolution.join(",") + ']' : '') + '</title>\r\n');
